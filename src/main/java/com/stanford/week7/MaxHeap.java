...
public class MaxHeap <T extends Comparable<T>> {
    ...
    private void heapifyDown() {
        ...
        while (hasLeftIndex(localIndex)) {
            largestIndex = getLeftIndex(localIndex);
            int rightIndex = getRightIndex(localIndex);
            if (rightIndex <= size - 1) {
                if (heap.get(largestIndex).compareTo(heap.get(rightIndex)) == -1)
                    largestIndex = getRightIndex(localIndex);
            }
            if (heap.get(localIndex).compareTo(heap.get(largestIndex)) == -1)
                swap(localIndex, largestIndex);
            localIndex = largestIndex;
        }
    }
    ...
    public void remove(T targetVertex) {
